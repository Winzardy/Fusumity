...
namespace Fusumity.Editor
{
	...
	public sealed class SchedulePointWindowDrawer : OdinValueDrawer<SchedulePoint>
	{
		...
		/// <summary>
		/// Тугл окна: выключен — поля не рисуются, длительность обнулена. Своё состояние держит
		/// только на нулевой длительности — «включили, но ещё не ввели»
		/// </summary>
		...
		protected override void DrawPropertyLayout(GUIContent label)
		{
			...
			// Тугл ведомый: есть длительность — окно есть, что бы дровер ни помнил раньше.
			// Иначе окно, заведённое из календаря или вернувшееся по undo, оставалось бы
			// в инспекторе выключенным, а его поля — скрытыми
			var active = duration > 0 || (_active ??= false);
