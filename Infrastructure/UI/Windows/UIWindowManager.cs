...
namespace UI.Windows
{
	public enum WindowMode
	{
		...
		[Tooltip("Overlay-режим: окно открывается поверх текущего, не закрывая его")]
		Overlay
	}
	...
	public partial class UIWindowManager : IInitializable, IDisposable
	{
		...
		private void Show([NotNull] IWindow window, in WindowQueueContext context, bool fromQueue = false)
		{
			...
			// Окно уже висит в очереди (перекрыто другим окном) и при этом остаётся Active.
			// Повторный показ упрётся в ранний return в UIWindow.Show (Active + те же args) и не поднимет
			// окно наверх. Сигналим, чтобы такой кейс не проходил незаметно — окно надо закрыть перед повторным показом
			if (!fromQueue && _queue.Contains(window))
				GUIDebug.LogWarning($"Окно [ {window.Id} ] показывается повторно, пока оно ещё в очереди (перекрыто). " +
					"Оно может не подняться наверх — закройте его перед повторным показом");
